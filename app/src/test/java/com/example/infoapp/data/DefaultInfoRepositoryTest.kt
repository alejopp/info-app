...
class DefaultInfoRepositoryTest {
    ...
    fun infos_newItemSaved_itemIsReturned() = runTest {
        ...
        assertEquals(repository.infos.first().size, 1)
    }

}

private class FakeInfoDao : InfoDao {

    private val data = mutableListOf<Info>()

    override fun getInfoList(): Flow<List<Info>> = flow {
        emit(data)
    }

    override suspend fun insertInfo(item: Info) {
        data.add(0, item)
    }
}
